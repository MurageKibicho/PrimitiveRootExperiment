...
//clear && gcc 3.4KibichoPohlig.c -lm -lgmp -lmpfr -lflint -o m.o && ./m.o
...
int SortbyD(const void *a, const void *b)
{
	const Elliptic ai_a = *(const Elliptic *)a;
	const Elliptic ai_b = *(const Elliptic *)b;
	return fmpz_cmp(ai_a->D, ai_b->D);
}

int SortbyLX(const void *a, const void *b)
{
	const Elliptic ai_a = *(const Elliptic *)a;
	const Elliptic ai_b = *(const Elliptic *)b;
	if(ai_a->legendre == ai_b->legendre)
	{
		return fmpz_cmp(ai_a->x, ai_b->x);
	}
	return ai_b->legendre - ai_a->legendre;
...
void CopyPoint(Elliptic source, Elliptic destination)
{
	...
	destination->legendre = source->legendre;
...
{
	printf("(%d)(x:",point->legendre);fmpz_print(point->x);printf(" y:");fmpz_print(point->y);
...
void Pohlig(Elliptic generator, Elliptic target, fmpz_t primeNumber, fmpz_t pointOrder, fmpz_t pointOrderMinusOne, fmpz_t mersenneGenerator, fmpz_factor_t factorization)
{
	...
	for(int exponent = 0; exponent < fmpz_get_ui(pointOrderMinusOne); exponent++)
	{
		...
		SolveD(result, primeNumber);
		result->exponent = exponent;//
		//result->exponent = fmpz_get_ui(privateKey);
		//if(fmpz_cmp_ui(privateKey, 2228) == 0)
		{
			/*fmpz_print(mersenneGenerator); printf(" ^ %d : (",exponent);
			fmpz_print(privateKey); printf(") ");
			for(slong i = 0; i < factorization->num; i++)
			{
				fmpz_mod(temp0, result->D, &factorization->p[i]);
				fmpz_print(temp0);
				printf(", ");
			}
			PrintPoint(result);*/
			Elliptic pointSave = CreatePoint();
			CopyPoint(result,pointSave);
			//PrintPoint(pointSave);
			arrput(points, pointSave);
		}
	}
	//qsort(points, arrlen(points), sizeof(Elliptic), SortbyD);
	for(size_t i = 0; i < arrlen(points); i++)
	{
		printf("%3d:  ", points[i]->exponent);PrintPoint(points[i]);
		//printf("%3ld:  ", i);PrintPoint(points[i]);
		//printf("%3d, %3ld\n", points[i]->exponent, fmpz_get_ui(temp0));//PrintPoint(points[i]);
	}
	for(size_t i = 0; i < arrlen(points); i++){DestroyPoint(points[i]);}arrfree(points);
