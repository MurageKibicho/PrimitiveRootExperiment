...
//clear && gcc 1.2.BothSides.c -lm -lgmp -lmpfr -lflint -o m.o && ./m.o
...
void BothSideScalarMultiplication(Elliptic resultant, Elliptic generator, fmpz_t privateKey, fmpz_t primeNumber)
{
	//Create pointAtInfinity
	Elliptic pointAtInfinityLSB = CreatePoint();
	Elliptic pointAtInfinityMSB = CreatePoint();
	pointAtInfinityMSB->infinity = 1; 
	pointAtInfinityLSB->infinity = 1;  
	
	Elliptic tempLSB0 = CreatePoint();
	Elliptic tempMSB0 = CreatePoint();
	//Save generator to tempLSB0
	CopyPoint(generator, tempLSB0); 
	Elliptic tempLSB1 = CreatePoint();
 
	size_t binaryLength = fmpz_sizeinbase(privateKey, 2);
	for(ssize_t i = binaryLength - 1, j = 0; i >= 0; --i, j++)
	{
		int LSB = fmpz_tstbit(privateKey, j);
		int MSB = fmpz_tstbit(privateKey, i);
		
		if(LSB)
		{
			AddCurvePoints(tempLSB1, pointAtInfinityLSB, tempLSB0, primeNumber);
			CopyPoint(tempLSB1, pointAtInfinityLSB);
		}
		
		AddCurvePoints(tempLSB1, tempLSB0, tempLSB0, primeNumber);
		AddCurvePoints(tempMSB0, pointAtInfinityMSB, pointAtInfinityMSB, primeNumber);
		CopyPoint(tempMSB0, pointAtInfinityMSB);
		CopyPoint(tempLSB1, tempLSB0);
		if(MSB)
		{
			AddCurvePoints(tempMSB0, pointAtInfinityMSB, generator, primeNumber);
			CopyPoint(tempMSB0, pointAtInfinityMSB);
		}
		//printf("\n(%ld %ld)",i,j);
		//fmpz_print(pointAtInfinityMSB->x);
		//printf("\n");
		//fmpz_print(pointAtInfinityLSB->x);
		//printf("\n");
	}
	
	assert(TestPointEquality(pointAtInfinityMSB, pointAtInfinityLSB));
	//printf("\n");
	//fmpz_print(pointAtInfinityMSB->x);
	//printf("\n");
	//fmpz_print(pointAtInfinityLSB->x);
	...
	CopyPoint(pointAtInfinityLSB, resultant);
	DestroyPoint(pointAtInfinityLSB);
	DestroyPoint(pointAtInfinityMSB);
	DestroyPoint(tempLSB0);
	DestroyPoint(tempMSB0);
	DestroyPoint(tempLSB1);
}
void BothSideBenchmark()
{
	int keyCount = 100000;
	...
	for(int i = 1; i < keyCount; i++)
	{
		...
		BothSideScalarMultiplication(result, generator, privateKey, primeNumber);	
	}
	PrintPoint(result);
	end = clock(); 
